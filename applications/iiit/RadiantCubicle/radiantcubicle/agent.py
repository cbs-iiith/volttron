...
def radiantcubicle(config_path, **kwargs):
    ...
    # This agent needs to be named iiit.radiantcubicle. Pop the uuid id off
    # the kwargs
...
class RadiantCubicle(Agent):
    ...
    def _rpcset_rc_auto_cntrl(self, state):
        ...
        try:
            ...
            _log.exception(
                'setting RC_AUTO_CNTRL, message: {}'.format(e.message))
            # print(e)
        finally:
            # cancel the schedule
            cancel_task_schdl(self, task_id)
    ...
    def _update_rc_tsp(self, new_tsp):
        ...
        # check if the new tsp really updated at the h/w, only then proceed
        # with new tsp
