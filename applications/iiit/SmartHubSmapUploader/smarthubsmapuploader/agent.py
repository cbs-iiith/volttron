...
# If you want to listen to all topics just replace the below with empty
# string ''
...
# agents whose published data to volttron bus we are interested in uploading
# to smap
...
def smarthubsmapuploader(config_path, **kwargs):
    ...
    class SmartHubSmapUploader(Agent):
        ...
        def startup(self, sender, **kwargs):
            ...
            self.vip.pubsub.subscribe('pubsub', self.sh_main_topic,
                                      self.on_match_shData)
            # subscribing to vb_main_topic
            self.vip.pubsub.subscribe('pubsub', self.vb_main_topic,
                                      self.on_match_shCurrentPP)
        ...
        def on_match_shCurrentPP(self, peer, sender, bus, topic, headers,
                                 message):
        ...
        def shSmapPostData(self, peer, sender, bus, topic, headers, message):
            ...
            # published by 'iiit.smarthub' or by 'iiit.volttronbridge' or by  
            'iiit.smarthubgc'
            if sender != self.sender_sh and sender != self.sender_vb and 
            sender != self.sender_gc:
            ...
            if 'sensors' in topic:
                ...
                # strip 'all' from the topic, don't know how to post all (
                # lux, rh, temp, co2 & pir) to the smap
                # so posting individually
                # topic = (topic.split('all', 1))[0]

                topic = '/SmartHub/' + self.sh_id + '/Sensors/lux'
                self.smapPostSensorsData('luxlevel', topic, headers, message,
                                         msg_time)

                topic = '/SmartHub/' + self.sh_id + '/Sensors/rh'
                self.smapPostSensorsData('rhlevel', topic, headers, message,
                                         msg_time)

                topic = '/SmartHub/' + self.sh_id + '/Sensors/temperature'
                self.smapPostSensorsData('templevel', topic, headers, message,
                                         msg_time)

                topic = '/SmartHub/' + self.sh_id + '/Sensors/co2'
                self.smapPostSensorsData('co2level', topic, headers, message,
                                         msg_time)

                topic = '/SmartHub/' + self.sh_id + '/Sensors/occupancy'
                self.smapPostSensorsData('pirlevel', topic, headers, message,
                                         msg_time)
                return
                ...
                smap_post(self.smap_root, self.api_key, topic, units,
                          reading_type, readings, self.source_data,
                          self.time_zone)
                return
                ...
                _log.debug(
                    'smapPostSSData() - Energy Demand - SmartStrip - ('
                    'DownStream) - YET TO IMPLEMENT!!!!!')
                return
