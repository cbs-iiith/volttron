...
def smartstripui(config_path, **kwargs):
    ...
    class SmartStripUI(PublishMixin, BaseAgent):
        ...
        def on_match_currentPP(self, peer, sender, bus, topic, headers, message):
            uiPostCurrentPricePoint(headers, message)
 
        @PubSub.subscribe('pubsub', plug1_meterData_all_point)
        def on_match_plug1MeterData(self, peer, sender, bus, topic, headers, message):
            uiPostMeterData(PLUG_ID_1, headers, message)
            
        @PubSub.subscribe('pubsub', plug2_meterData_all_point)
        def on_match_plug2MeterData(self, peer, sender, bus,  topic, headers, message):
            uiPostMeterData(PLUG_ID_2, headers, message)

        @PubSub.subscribe('pubsub', plug1_relayState_point)
        def on_match_plug1MeterData(self, peer, sender, bus,  topic, headers, message):
            uiPostRelayState(PLUG_ID_1, headers, message)

        @PubSub.subscribe('pubsub', plug2_relayState_point)
        def on_match_plug2MeterData(self, peer, sender, bus,  topic, headers, message):
            uiPostRelayState(PLUG_ID_2, headers, message)

        @PubSub.subscribe('pubsub', plug1_thresholdPP_point)
        def on_match_plug1Threshold(self, peer, sender, bus,  topic, headers, message):
            uiPostThreshold(PLUG_ID_1, headers, message)

        @PubSub.subscribe('pubsub', plug2_thresholdPP_point)
        def on_match_plug2Threshold(self, peer, sender, bus,  topic, headers, message):
            uiPostThreshold(self, PLUG_ID_2, headers, message)
...
def main(argv=sys.argv):
    ...
    try:
        ...
        description='VOLTTRON platform™ agent for user interaction.',
        argv=argv)
