...
class SmartHubUI_Clnt(Agent):
    ...
    def setup(self, sender, **kwargs):
        ...
        self._url_root = 'http://' + ble_ui_srv_address + ':' + str(
            ble_ui_srv_port) + '/smarthub'
    ...
    def _config_get_points(self):
        self.topic_price_point = self.config.get('topic_price_point',
                                                 'smarthub/pricepoint')
        self.topic_sensors = self.config.get('sensorsLevelAll_point',
                                             'smarthub/sensors/all')
        self.topic_led_state = self.config.get('ledState_point',
                                               'smarthub/ledstate')
        self.topic_fan_state = self.config.get('fanState_point',
                                               'smarthub/fanstate')
        self.topic_led_level = self.config.get('ledLevel_point',
                                               'smarthub/ledlevel')
        self.topic_fan_level = self.config.get('fanLevel_point',
                                               'smarthub/fanlevel')
        self.topic_led_th_pp = self.config.get('ledThPP_point',
                                               'smarthub/ledthpp')
        self.topic_fan_th_pp = self.config.get('fanThPP_point',
                                               'smarthub/fanthpp')
        return
    ...
    def _rpc_sensors_data(self, sender, headers, message):
        ...
        do_rpc(self._agent_id, self._url_root
               , 'sensors',
               {'lux': lux, 'rh': rh, 'temp': temp, 'co2': co2, 'pir': pir}
               , 'POST')
        return
