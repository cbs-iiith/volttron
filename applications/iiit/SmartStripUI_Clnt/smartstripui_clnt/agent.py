...
def smartstripui_clnt(config_path, **kwargs):
    ...
    class SmartStripUI_Clnt(Agent):
        ...
        def setup(self, sender, **kwargs):
            ...
            ble_ui_srv_address = config.get('ble_ui_server_address',
                                            '127.0.0.1')
            ble_ui_srv_port = config.get('ble_ui_server_port', 8081)
            self.url_root = 'http://' + ble_ui_srv_address + ':' + str(
                ble_ui_srv_port) + '/smartstrip'
        ...
        def _configGetPoints(self):
            ...
            self.plug1_meterData_all_point = config.get(
                'plug1_meterData_all_point',
                'smartstrip/plug1/meterdata/all')
            self.plug2_meterData_all_point = config.get(
                'plug2_meterData_all_point',
