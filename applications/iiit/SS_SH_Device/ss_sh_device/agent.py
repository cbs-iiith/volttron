...
def ss_sh_device(config_path, **kwargs):
    ...
    '''
    This agent runs on the smart strip and controls the plug to which the 
    smart hub is connected.
        
    sh_btry_th = 1 if the battery charge level is above a certain charge 
    threshold
               = 0
    ...
    Process-I (when PP changes, subscribe to ss_pp_topic)
        If the price point is more than the SmartHub threshold pricepoint && 
        sh_btry_th = 1, 
            change th_pp to 1 for the plug to which hub is connected (ss will 
            switch-off the plug)
            sh_on_btry = True
            if sh_on_btry_start_time != 0
                charge_time = now() - sh_on_btry_start_time
                sh_brty_level = sh_brty_level -  charge_time * charge_rate (
                decrease the btry level)
            sh_on_btry_start_time = now()
                
        If the price point is less than the SmartHub threshold pricepoint
            change th_pp to 0 for the plug to which hub is connected (ss will 
            switch-on the plug)
            sh_on_btry = False
            if sh_on_grid_start_time != 0
                charge_time = now() - sh_on_grid_start_time
                sh_brty_level = sh_brty_level +  charge_time * charge_rate (
                increase the btry level)
    ...
    Process-III (at regular interval)
        ...
        if running on battery for more than 2 hours
               sh_btry_th = 0 
        after certain amount of time (when batter is charged to a threshold 
        limit) sh_btry_th = 1
    ...
    class SS_SH_Device(Agent):
        ...
        def startup(self, sender, **kwargs):
            ...
            self.core.periodic(self.period_read, self._monitorShBattery,
                               wait=None)
            self.vip.pubsub.subscribe('pubsub', self.ss_pp_topic,
                                      self.on_new_price)
            return
        ...
        def _configGetInitValues(self):
            ...
            self.ss_pp_topic = config.get('ss_pp_topic',
        ...
        def on_new_price(self, peer, sender, bus, topic, headers, message):
            ...
            if not sh_on_btry and new_price_point > self.sh_th_pp and \
                    sh_btry_th == 1:
                # change th_pp to max for the plug to which hub is connected
                # (ss will switch-off the plug)
            ...
            if sh_on_btry and new_price_point <= self.sh_th_pp:
                # change th_pp to min for the plug to which hub is connected
                # (ss will switch-on the plug)
        ...
        def _pingSmartHub(self):
            ...
            url_root = 'http://' + self.sh_ip_addr + ':' + str(
                self.sh_port) + '/smarthub'
            ...
            if self.hub_ping_count > MAX_RETRIES:
                ...
                if plugOn:
                    _log.debug(
                        'thpp updated successfuly, hopefull ss will switch-on '
                        'power!!!')
                    sh_on_btry = False
                    sh_on_grid_start_time = datetime.datetime.utcnow()
                else:
                    _log.debug(
                        'failed to switch on power, will try in next iteration')
                    self.hub_ping_count = 0
