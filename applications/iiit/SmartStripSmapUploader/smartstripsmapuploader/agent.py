...
def smartstripsmapuploader(config_path, **kwargs):
    ...
    class SmartStripSmapUploader(Agent):
        ...
        def startup(self, sender, **kwargs):
            _log.debug('startup()')
            self.vip.pubsub.subscribe('pubsub', self.ss_main_topic,
                                      self.on_match_ssData)
            self.vip.pubsub.subscribe('pubsub', self.vb_main_topic,
                                      self.on_match_ssCurrentPP)
            return
        ...
        def on_match_ssCurrentPP(self, peer, sender, bus, topic, headers,
                                 message):
        ...
        def ssSmapPostData(self, peer, sender, bus, topic, headers, message):
            ...
            # published by 'iiit.smartstrip' or by 'iiit.volttronbridge' or
            # by 'iiit.smartstripgc'
            '''
            if sender != self.sender_ss and sender != self.sender_vb and 
            sender != 'iiit.smartstripgc':
            ...
            if 'meterdata' in topic:
                ...
                topic = '/SmartStrip/' + self.ss_id + \
                        '/meterdata/active_power/' + plug_id
                self.smapPostMeterData('active_power', topic, headers, message,
                                       msg_time)
                return
                ...
                _log.debug(
                    'smapPostSSData() - PricePoint - Smarthub (Upstream)')
